...
function scan_dir_filter($dir, $filter = "") {
    $list = scandir($dir);
     // on scan le dossier
    $filterDefault = array(".", "..", "__MACOSX", "nbproject", "_notes", ".DS_Store", ".komodotools", "_tmp", "_INC_", "_EXEC_",".git");
...
function file_list_mono($dir, $suffix = null) {
    $filter = array(".", "..", "__MACOSX", "nbproject", "_notes", ".DS_Store", ".komodotools", "_tmp", "_INC_", "_EXEC_",".git");
     //list des nom de fichier ou de dossier a ne pas indexer
    ...
    foreach ($r as $key => $val) {
        ...
        } else $r[$key] = ($suffix != null) ? $suffix . $val : $val;
    }
    return $r;
}

function folder_list($dir, $rel = true) {
    $filter = array(".", "..", "__MACOSX", "nbproject", "_notes", ".DS_Store", ".komodotools", "_tmp", "_INC_", "_EXEC_", "_TEMPLATES_",".git");
     //list des nom de fichier ou de dossier a ne pas indexer
    $list = scandir($dir);
     // on scan le dossier
    $r = array_diff($list, $filter);
...
function templateFile($templateFile, $targetDir, $newFileName = "") {
    ...
    if (!file_exists($dest)) return copy($src, $dest);
    else return false;
}
